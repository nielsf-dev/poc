...
public class NIONonBlockingPollingServer {
    ...
    public static void main(String[] args) throws IOException {
        ...
        final Map<SocketChannel, ByteBuffer> socketByteBuffers = new ConcurrentHashMap<>();
        ...
        while (true) {
            ...
            if (socket != null) {
                ...
                socketByteBuffers.put(socket, ByteBuffer.allocateDirect(80));
            }

            socketByteBuffers.keySet().removeIf((socketChannel) -> !socketChannel.isOpen());
            // Remove socketChannel which is not opened

            // Iterate through sockets to see f any socket has anything to say
            socketByteBuffers.forEach((socketCh, byteBuffer) -> {
                try {
